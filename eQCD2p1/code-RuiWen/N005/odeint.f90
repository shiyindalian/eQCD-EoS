...
!User storage for intermediate results. Preset dxsav and kmax
  ...
  do nstp=1,MAXSTP
    ...
    yscal(1:nvar)=abs(y(1:nvar))+abs(h*dydx(1:nvar))+TINY
    if(kmax.gt.0)then
...
!derivs(x,y,dydx), which returns derivatives dydx at x.
  ...
  PARAMETER (A2=.2Q+0,A3=.3Q+0,A4=.6Q+0,A5=1.Q+0,A6=.875Q+0,B21=.2Q+0,B31=3.Q+0/40.Q+0, &
             ...
             DC6=C6-.25Q+0)
  ytemp(1:n)=y(1:n)+B21*h*dydx(1:n)
  call derivs(x+A2*h,ytemp,ak2)
  ytemp(1:n)=y(1:n)+h*(B31*dydx(1:n)+B32*ak2(1:n))
  call derivs(x+A3*h,ytemp,ak3)
  ytemp(1:n)=y(1:n)+h*(B41*dydx(1:n)+B42*ak2(1:n)+B43*ak3(1:n))
  call derivs(x+A4*h,ytemp,ak4)
  ytemp(1:n)=y(1:n)+h*(B51*dydx(1:n)+B52*ak2(1:n)+B53*ak3(1:n)+B54*ak4(1:n))
  call derivs(x+A5*h,ytemp,ak5)
  ytemp(1:n)=y(1:n)+h*(B61*dydx(1:n)+B62*ak2(1:n)+B63*ak3(1:n)+B64*ak4(1:n)+B65*ak5(1:n))
  call derivs(x+A6*h,ytemp,ak6)
  yout(1:n)=y(1:n)+h*(C1*dydx(1:n)+C3*ak3(1:n)+C4*ak4(1:n)+C6*ak6(1:n))
  yerr(1:n)=h*(DC1*dydx(1:n)+DC3*ak3(1:n)+DC4*ak4(1:n)+DC5*ak5(1:n)+DC6*ak6(1:n))
  return
END
!  (C) Copr. 1986-92 Numerical Recipes Software ,4-#.
