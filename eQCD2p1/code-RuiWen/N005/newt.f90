subroutine newt(x, n, check, functionn)
  ...
  parameter (NP=40, MAXITS=200, TOLF=1.Q-5, TOLMIN=1.Q-7, TOLX=1.Q-8, STPMX=100.Q+0)
...
!  f=fmin(x)                         !The vector fvec is also computed by this call
  ...
  test=0.Q+0                           !Test for initial guess being a root. Use more stringent
  do i=1, n                         !test than simply TOLF
    if(abs(fvec(i)).gt.test)test=abs(fvec(i))
  end do

  if(test.lt..01Q+0*TOLF) then
    check=.false.
	return
  end if

  sum=0.Q+0                            !Calculate stpmax for line searches
  do i=1, n
    sum=sum+x(i)**2
  end do
...
!lnsrch returns new x and f. It also calculates fvec at the new x when it calls fmin
    ...
    test=0.Q+0                            !Test for convergence on function values
	do i=1, n
	  if(abs(fvec(i)).gt.test)test=abs(fvec(i))
	end do
	...
	if(check) then                     !Check for gradient of f zero, i.e., spurious convergence
	  ...
	  den=max(f,.5Q+0*n)
	  do i=1, n
	    temp=abs(g(i))*max(abs(x(i)),1.Q+0)/den
...
subroutine lnsrch(n, xold, fold, g, p, x, f, stpmax, check, func, functionn)
  ...
  parameter (ALF=1.Q-5,TOLX=1.Q-8)
...
subroutine ludcmp(a, n, np, indx, d)  !¾ØÕóµÄLU·Ö½â
  ...
  parameter (NMAX=10,TINY=1.0Q-20)
  integer i, imax, j, k
